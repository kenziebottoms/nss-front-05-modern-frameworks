# Modern Front End Frameworks

A repo for notes and throwaway code associated with the front-end half of Nashville Software School, based on [this repo](https://github.com/nashville-software-school/front-end-milestones/tree/master/5-modern-frameworks).

---

## AngularJS 101: Get Started

[NSS Study Guide: AngularJS 101](learning-materials/MF_ANGULAR_101.md)

   ### Read these articles first
   * [AngularJS for beginners](http://medialoot.com/blog/angularjs-for-absolute-beginners/)
   * [AngularJS tutorial](https://www.airpair.com/angularjs/posts/angularjs-tutorial)
   * [AngularJS in one day](http://toddmotto.com/ultimate-guide-to-learning-angular-js-in-one-day/)

   ### Online exercises
   * [A Better Way to Learn AngularJS](https://thinkster.io/a-better-way-to-learn-angularjs/)
   * [AngularJS on Code Academy](https://www.codecademy.com/courses/javascript-advanced-en-2hJ3J/0/1)

   ### Quick reference
   [AngularJS in 30 Minutes](http://www.revillweb.com/tutorials/angularjs-in-30-minutes-angularjs-tutorial/)

   ### NSS Exercises
   * [Mushrooms](learning-materials/MF_MUSHROOMS.md)
   * [Wide World Travel](learning-materials/MF_WIDE_WORLD_TRAVEL.md)

---

## AngularJS 102: Routes

   [NSS Study Guide: AngularJS 102](learning-materials/MF_ANGULAR_102.md)

   ### NSS Exercises
   * [Routing](learning-materials/MF_ROUTE_ROUTING.md)
   * [Listing](learning-materials/MF_ROUTE_LISTING.md)
   * [Mushroom Factory](learning-materials/MF_MUSHROOM_FACTORY.md)
   * [Wide World Travel Factory](learning-materials/MF_WIDE_WORLD_TRAVEL_FACTORY.md)
   * [Wide World Travel Routing](learning-materials/MF_WIDE_WORLD_TRAVEL_ROUTING.md)

---

## AngularJS 103: Connecting with Firebase


   [NSS Study Guide: AngularJS 103](learning-materials/MF_ANGULAR_103.md)

   ### NSS Exercies
   [User Notes](learning-materials/MF_USER_NOTES.md)

---

## AngularJS 104: controllerAs and UI router

   [NSS Study Guide: AngularJS 104](learning-materials/MF_ANGULAR_104.md)

   ### Online Resources
   * [Intro to controller as](http://toddmotto.com/digging-into-angulars-controller-as-syntax/)
   * [AngularJS UI router](https://github.com/angular-ui/ui-router) is a more advanced routing library that isn't solely dependent upon the URL, and supports nested views.

---

## AngularJS 105: Directives

   [NSS Study Guide: AngularJS 105](learning-materials/MF_ANGULAR_105.md)

   ### Online Resources
   * [Angular directives tutorial](http://www.befundoo.com/university/tutorials/angularjs-directives-tutorial/)
   * [Create a tree hierarchy with ng-include](http://gurustop.net/blog/2014/07/15/angularjs-using-templates-ng-include-create-infinite-tree/)
   * [How to Work With All the Scopes in Angular](http://jonathancreamer.com/working-with-all-the-different-kinds-of-scopes-in-angular/)

   ### NSS Exercises
   * [Doctors & Patients](learning-materials/MF_DOCTORS_PATIENTS.md)
   * [National Parks](learning-materials/MF_NATIONAL_PARKS.md)
   * [Nashville Chart](learning-materials/MF_MF_NASHVILLE_CHART.md)


---

## AngularJS 106: AngularJS with Browserify

### Online Resources
   [AngularJS And Browserify — The Dream Team For Single Page Applications](https://blog.codecentric.de/en/2014/08/angularjs-browserify/)

---
## AngularJS Interpolation


[NSS Study Guide: AngularJS Interpolation](learning-materials/MF_ANGULAR_INTERPOLATION.md)